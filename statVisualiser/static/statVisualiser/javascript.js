...
function HideAll(id) {
  ...
  document.getElementById('picker' + id + '-Probability').style.display =
    'none';
  ...
  for (let i = 0; i < labels.length; i++) {
    if (labels[i].htmlFor.includes('picker' + id) && !(labels[i].htmlFor
        .includes('Type'))) {
...
function Hide(id) {
  HideAll(id);
  let name = document.getElementById('picker' + id + '-Type').options[document
    .getElementById('picker' + id + '-Type').selectedIndex].innerHTML
  if (name !== '') {
    const list = JSON.parse(document.getElementById(name).innerHTML.replace(
      /'/g, '"'));
    let labels = document.getElementsByTagName('label');
    for (let i = 0; i < list.length; i++) {
      for (let j = 0; j < labels.length; j++) {
        if (labels[j].htmlFor.includes('picker' + id + '-' + list[i]) || labels[
            j].htmlFor.includes('picker' + id + '-' + 'Output') || labels[j]
          .htmlFor.includes('picker' + id + '-' + 'G_Min') || labels[j].htmlFor
          .includes('picker' + id + '-' + 'G_Max')) {
